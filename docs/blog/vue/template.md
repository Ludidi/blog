...
> 源码在 src/compiler 目录下

![image.png](/images/vue/template/image-1.png)

先来查看官方提供的生命周期图片：

1. 渲染到哪个根节点，判断有无 el 属性，有的话直接取 el 根节点，没有的话则调用$mount 去获取根节点
1. 渲染到模版
   1. 有 render：优先执行 render 函数
   1. 无 render：
      1. 有 template：拿 template 解析成 render 函数所需要的格式，并调用 render 函数渲染
      1. 无 template：拿 el 的根节点的 outerHTML 去解析成 render 函数所需的格式，并调用 render 函数渲染
1. 最终都会统一使用 render 函数渲染

## 实现步骤

1. $mount 函数
1. 解析 template 为抽象语法树（AST）
1. 将抽象语法书（AST）转为 render 所需要的格式
...
> 为函数编译的入口函数，包含 parse 和 generate 的执行，返回值是一个 render 函数
...
> 将 template 转为抽象语法树（AST）
>
> - 首先需要各种正则匹配的正则表达式（开始标签，结束标签，花括号等）
...
> 将 AST 转为 render 函数格式的数据
>
> - 匹配花括号
> - 确保 AST 解析为 render 函数所需格式
...
![image.png](/images/vue/template/image-2.png)

## 参考

[https://juejin.cn/post/6969563640416436232](https://juejin.cn/post/6969563640416436232)
[https://juejin.cn/post/6936024530016010276](https://juejin.cn/post/6936024530016010276#heading-7)
